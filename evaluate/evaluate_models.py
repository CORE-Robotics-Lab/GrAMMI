...
def evaluate_model(model_folder_path, mode="mixture"):
    """
    Evaluate the current model for all the metrics on its test or validation dataset
    :param model_folder_path - Path to the stored model file (By default, uses the best model)
    :param mode - "mixture" or "single", Evaluates each metric either with respect to the full Gaussian mixture
                   or with respect to a single Gaussian with the highest mixing coefficient
    """
    ...
    for x_test, y_test in tqdm.tqdm(test_dataloader):
        ...
        if mode == "mixture":
            logprob, ade, dist_thresh, one_sigma, two_sigma, three_sigma, num_steps = evaluate_mixture_metrics(nn_output,
                                                                                                               y_test)
        else:
            logprob, ade, dist_thresh, one_sigma, two_sigma, three_sigma, num_steps = evaluate_mode_metrics(nn_output,
                                                                                                            y_test)
...
def write_metric_logs(model_paths, writer, eval_mode):
    for model_path in model_paths:
        metrics = evaluate_model(model_path, mode=eval_mode)
