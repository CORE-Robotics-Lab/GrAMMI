...
def train(seed, 
    ...
    for epoch in tqdm(range(1, n_epochs+1)):
        ...
        for tup in train_dataloader:
            ...
            if config["datasets"]["dataset_type"] == "padded":
            ...
            elif config["datasets"]["dataset_type"] == "padded_agent_gnn":
                ...
                x_train = (x_train, agent_packed, n_agents)
            else:
                x_train, y_train = tup
            ...
            # x_embed = torch.stack([trajectory_specific_embeddings[idx] for idx in x_embed_idx])
            # if config["datasets"]["dataset_type"] == "padded":
            out, posterior_input, pi_cats = model.forward_encoder(x_train)
        ...
        if epoch % 2 == 0:
            ...
            for tup_test in test_dataloader:
                ...
                num_batches_test += 1
                # x_embed_test = torch.stack([test_specific_embeddings[idx] for idx in x_embed_idx])
                # if config["datasets"]["dataset_type"] == "padded":
                    # out, _ = model(x_test)
                out, posterior_input, pi_cats = model.forward_encoder(x_test)
                post = posterior(*posterior_input)
                # else:
                    # out = model(x_test)
...
def main_config_mutual_info(config):
