        ...
        for b in range(bs):
            for s in range(seq_len):
                ...
                for i in range(num_mixtures):
                    probs[b, s, i] = multivariate_normal.cdf(target[b, s, i] + dist_threshold, mean=mu[b, s, i],
                                                             cov=cov[b, s, i]) - \
                                     multivariate_normal.cdf(target[b, s, i] - dist_threshold, mean=mu[b, s, i],
                                                             cov=cov[b, s, i])
...
class VAEOpponentDecoderMultiHead(MixureDecoderMultiHead):
    def __init__(self, input_dim, output_dim=2, num_gaussians=2, num_heads=1, log_std_init=0.0, beta=1.0):
        super(VAEOpponentDecoderMultiHead, self).__init__(input_dim=input_dim, output_dim=output_dim,
                                                          num_gaussians=num_gaussians, num_heads=num_heads,
    ...
    def forward(self, x):
        concat_embed, self.z_mean, self.z_std = x
    ...
    def compute_loss(self, nn_output, red_locs):
        ...
        losses = self.mdn_negative_log_likelihood_loss(nn_output, red_locs)
        loss = torch.sum(losses, dim=1).mean() + self.beta * self.compute_kl_loss()
        return loss

    def compute_kl_loss(self):
        # TODO Check if this computation is correct
        # kld_loss = torch.mean(-0.5 * torch.sum(1 + self.z_log_std - self.z_mean**2 - self.z_log_std.exp(), dim=1), dim=0)
        # return kld_loss

        ideal_std = torch.ones_like(self.z_std)
        EPS = torch.finfo(torch.float).eps

        # KL Divergence between predicted latent distribution (G1) and ideal gaussian (G2)

        kld_loss = 0.5 * (2 * torch.log(ideal_std + EPS) - 2 * torch.log(self.z_std + EPS) +
                          (self.z_std.pow(2) + self.z_mean.pow(2)) /
                          ideal_std.pow(2) - 1)

        kld_loss = torch.sum(kld_loss, dim=1).mean()

        return kld_loss


class CategoricalVAEDecoder(MixureDecoderMultiHead):
    def __init__(self, input_dim, output_dim=2, num_gaussians=2, num_heads=1, log_std_init=0.0,
                 beta=1.0, categorical_dim=4, latent_dim=8):
        super(CategoricalVAEDecoder, self).__init__(input_dim=input_dim, output_dim=output_dim,
                                                    num_gaussians=num_gaussians, num_heads=num_heads,
                                                    log_std_init=log_std_init)
        self.z = []
        self.beta = beta
    ...
    def compute_loss(self, nn_output, red_locs):
        ...
        # Currently only using single head for categorical VAE
        losses = self.mdn_negative_log_likelihood_loss(nn_output, red_locs)
        loss = torch.sum(losses, dim=1).mean() + self.beta * self.compute_kl_loss()
        return loss
    ...
    def compute_kl_loss(self):
        ...
        # kld_loss = torch.mean(-0.5 * torch.sum(1 + self.z_log_std - self.z_mean**2 - self.z_log_std.exp(), dim=1), dim=0)
        # return kld_loss
        self.z = self.z.view(-1, self.latent_dim, self.categorical_dim)
        z_p = F.softmax(self.z, dim=-1)  # Convert the categorical codes into probabilities

        # KL Divergence with Gumbel Softmax distribution
        EPS = torch.finfo(torch.float).eps

        # Entropy of the logits
        h1 = z_p * torch.log(z_p + EPS)

        # Cross entropy with the categorical distribution
        h2 = z_p * np.log(1. / self.categorical_dim + EPS)
...
class Seq2SeqContrastiveAutoregressiveDecoder(ContrastiveGNNMixureDecoderMultiHead):
    ...
    def __init__(self, input_dim, output_dim=2, num_gaussians=2, num_heads=1, log_std_init=0.0,
                 teacher_forcing=0.5):
        super(Seq2SeqContrastiveAutoregressiveDecoder, self).__init__(input_dim, output_dim, num_gaussians, num_heads,
                                                                      log_std_init)
