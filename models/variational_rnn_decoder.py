...
class VariationalMixtureDecoderMultiHead(nn.Module):
    ...
    def __init__(self, input_dim, output_dim=2, num_gaussians=2, num_heads=1,
                 kl_loss_wt=1.0, log_std_init=0.0, padded_input=False):
    ...
    def forward(self, x):
        if self.padded_input:
            concat_embed, self.all_enc_mean, self.all_enc_std, self.all_prior_mean, self.all_prior_std, x_lens = x
        else:
        ...
        if self.padded_input:
            return (pi, mu, sigma), x_lens
        return pi, mu, sigma

    def compute_loss(self, nn_output, red_locs):
        if self.padded_input:
            nn_output, x_lengths = nn_output
        else:
            x_lengths = []
        # nn_output = self.forward(features)
        red_locs = red_locs.view(-1, self.num_heads, self.output_dim)

        losses = self.mdn_negative_log_likelihood_loss(nn_output, red_locs) + self.compute_kl_loss(x_lengths)
    ...
    def compute_kl_loss(self, x_lengths=[]):
        """
        ...
        # TODO: Zero out the loss for padded inputs
        if self.padded_input:
            assert len(x_lengths) != 0
            # x_lengths = torch.Tensor(x_lengths).to(self.all_prior_mean[0].device).float()
            ...
            for t in range(len(self.all_prior_mean)):
                ...
                mask = (t < x_lengths).float().unsqueeze(1).expand_as(kld_element)
                kld_element = kld_element * mask
