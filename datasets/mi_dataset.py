...
class MIDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return x, self.trajectory_num[idx], y
...
def pad_collate_with_traj(batch):
    (xx, trajectory_nums, yy) = zip(*batch)
    x_lens = [len(x) for x in xx]
    xx_pad = pad_sequence(xx, batch_first=True, padding_value=0)

    yy = torch.stack(yy, axis=0)
    return xx_pad, x_lens, trajectory_nums, yy

if __name__ == "__main__":
    data_path = "/data/manisha/datasets/fixed_policy/corner_start_fixed_hideouts/test"
    dataset = MIDataset(data_path, input_type = "blue", output_type = "red_state")

    dataloader = DataLoader(dataset = dataset, batch_size = 100, shuffle=True, collate_fn=pad_collate_with_traj)

    for tup in dataloader:
        x, x_lens, trajectory_nums, y = tup
        break
